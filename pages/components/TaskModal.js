    ...
    >
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="mb-4 grid grid-cols-6 gap-4">
          {icons.map((icon) => (
              <div key={icon.icone_id}>
                ...
                <label htmlFor={`icon-${icon.icone_id}`} class="inline-flex items-center justify-between p-5 text-gray-500 bg-white border-2 border-gray-200 rounded-lg cursor-pointer dark:hover:text-gray-300 dark:border-gray-700 peer-checked:border-blue-600 hover:text-gray-600 dark:peer-checked:text-gray-300 peer-checked:text-gray-600 hover:bg-gray-50 dark:text-gray-400 dark:bg-gray-800 dark:hover:bg-gray-700">
        ...
        <div className="mb-4 flex flex-wrap gap-2">
          {status.map((sts) => (
              <div key={sts.status_id} className="w-[48%]">
                <input onChange={(e) => setTaskData({...taskData, status_id: parseInt(e.target.value)})} checked={taskData.status_id == sts.status_id} type="radio" id={`status-${sts.status_id}`} value={sts.status_id} class="hidden peer"/>
                <label htmlFor={`status-${sts.status_id}`} class="w-full inline-flex items-center justify-between p-5 text-gray-500 bg-white border-2 border-gray-200 rounded-lg cursor-pointer dark:hover:text-gray-300 dark:border-gray-700 peer-checked:border-blue-600 hover:text-gray-600 dark:peer-checked:text-gray-300 peer-checked:text-gray-600 hover:bg-gray-50 dark:text-gray-400 dark:bg-gray-800 dark:hover:bg-gray-700">
                  <div className="tasks_itens_status" dangerouslySetInnerHTML={{ __html: sts.sts_icon }} />
                  <span>{ sts.sts_name }</span>
                </label>
              </div>
            ))}
        </div>
